...
export default function FinalCTA({ onSignIn }: FinalCTAProps) {
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    if (email.trim()) {
      ...
      setTimeout(() => {
        onSignIn();
      }, 1000); // Trigger sign in after showing the success state briefly
    }
  };
        ...
        >
          {!submitted ? (
            <form onSubmit={handleSubmit} className="flex flex-col sm:flex-row items-center gap-3 max-w-md mx-auto">
              ...
              <button
                type="submit"
                className="relative group w-full sm:w-auto flex-shrink-0 px-6 py-3.5 rounded-xl overflow-hidden text-sm font-semibold"
              >
